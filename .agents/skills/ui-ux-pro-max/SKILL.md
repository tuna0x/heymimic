...
```bash
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "<keyword>" --stack <stack>
```
...
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
python "${CLAUDE_PLUGIN_ROOT}scripts/search.py" "suspense streaming bundle" --stack nextjs
```
