...
import { FlipCardFrame } from '../shared/FlipCardFrame'
...
export function FlashCard({ word, onRate }: FlashCardProps) {
  ...
  const front = (
    <div className="bg-study-surface-muted/50 border border-study-border/80 rounded-2xl p-6 min-h-[260px] flex flex-col justify-between">
      <div>
    ...
    <div className="bg-study-surface-muted/50 border border-study-border/80 rounded-2xl p-6 min-h-[260px] flex flex-col justify-between">
      <div>
        ...
        <div className="my-4">
          ...
          <p className="text-xs text-study-text-muted mt-2">{word.translation}</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col h-full justify-between">
      <FlipCardFrame flipped={flipped} front={front} back={back} />

      <button
        type="button"
        onClick={() => setFlipped((value) => !value)}
        className="motion-control w-full mt-4 py-2.5 rounded-xl border border-study-border hover:border-study-primary/40 bg-study-surface hover:bg-study-surface-hover text-xs font-semibold text-study-text flex items-center justify-center gap-2 cursor-pointer shadow-xs"
      >
      ...
      {onRate && (
        <div className="grid grid-cols-2 gap-3 mt-4">
          <button
            ...
            className={`motion-control py-2.5 px-3 rounded-xl font-semibold text-xs flex items-center justify-center gap-2 cursor-pointer ${
              !flipped
